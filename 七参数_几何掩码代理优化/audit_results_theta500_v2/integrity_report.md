...
| 关键结果数据来源 | 已满足 | 标签样本500组，数据划分{'train': 375, 'val': 75, 'test': 50}；消融结果5组，K折摘要存在=True，优化记录存在=True，COMSOL复核存在=True。 |
| 论文图表可追溯 | 已满足 | 论文图片25张、缺失7张；图题25个、表题13个；关键数值匹配8/12。 |
| 优化阶段不依赖CFD云图 | 已满足 | 关键限制表述检查项6个；禁用表述命中数0。 |
| 敏感性分析结论边界 | 已满足 | 当前主线标签样本500组，非零theta样本数为500；敏感性结论仍应表述为数据驱动分箱趋势和随机森林重要性。 |
| 三维验证结论边界 | 受限 | 三维结果记录2条；当前存在eta_3d<1的候选结构，不能写成三维综合最优；非零theta三维模型存在=True。 |

## 警告

- 三维倾斜角建模记录theta=9.27235911523071与二维候选theta=-7.16021645711553不一致
- f_R2 当前最高模型是 no_cbam，论文不得写 PI-CNN-CBAM 全指标最优
...
- 论文未找到关键数值: 二维COMSOL复核Nu=44.076848
- 论文未找到关键数值: 二维COMSOL复核f=0.10761
- 论文未找到关键数值: 二维COMSOL复核eta=1.1053258675186466
- 论文未找到关键数值: PI-CNN-CBAM Nu_R2=0.9578245878219604

## 剩余工作清单

| 优先级 | 事项 | 当前证据 | 下一步 |
|---|---|---|---|
| 中 | 改进f预测或保持论文限制表述 | 当前 f_R2 最高模型为 no_cbam，不是 pi_cnn_cbam。 | 若要强化PI-CNN-CBAM结论，应增加阻力相关特征或损失；否则正文保持不写全指标最优。 |
...
- 旧六参数脚本语法通过数：5
- 标签样本数：500
- eta参考基准样本：case_id=20362，Nu0=43.582033，f0=0.14048
- 数据划分：{'train': 375, 'val': 75, 'test': 50}
- theta 唯一值数量：500
...
- 二维非零theta候选角度：-7.16021645711553
...
| 名称 | 状态 | 路径 |
|---|---|---|
| 七参数标签 | 存在 | `七参数_几何掩码代理优化\samples\labels_7param_theta500_v2.csv` |
| 七参数数据划分 | 存在 | `七参数_几何掩码代理优化\samples\dataset_split_theta500_v2.csv` |
| 消融实验汇总 | 存在 | `七参数_几何掩码代理优化\ablation_results_theta500_v2\ablation_summary.csv` |
| K折汇总 | 存在 | `七参数_几何掩码代理优化\kfold_results_theta500_v2\kfold_summary.json` |
| PI-CNN-CBAM优化参数 | 存在 | `七参数_几何掩码代理优化\optimization_results_theta500_v2_gp\best_params.json` |
| COMSOL复核报告 | 存在 | `七参数_几何掩码代理优化\validation_results_theta500_v2_gp\final_validation_report.json` |
