...
"""
COMSOL 单次仿真执行器 (Worker) - 终极全特征提取版
【核心功能】：接收外部 6 维参数 -> 建立模型 -> 求解 -> 导出纯净三场云图 -> 提取 10 项底层物理特征
"""
...
def create_and_solve_thermal_model(args, outdir):
    # 强制将 Windows 路径转为 Java 兼容的格式，并创建文件夹
    ...
    try:
        ...
        sel_out.set('xmin', -0.05 + float(eval(rect_w_expr.replace('chord','0.01'))) - 0.001)
        sel_out.set('xmax', -0.05 + float(eval(rect_w_expr.replace('chord','0.01'))) + 0.001)
