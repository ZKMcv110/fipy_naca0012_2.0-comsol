...
```powershell
.\myenvs_fipynaca2.0\Scripts\python.exe .\data_processing\organize_dataset.py
```
...
这些脚本已经改成以项目根目录为路径基准，移动到 `data_processing/` 后仍然会读写根目录下的 `consol_cfddata/` 和 `csv_data/`。
