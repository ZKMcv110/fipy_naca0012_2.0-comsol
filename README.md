...
> 新入口：先看 [docs/10-项目说明书.md](docs/10-项目说明书.md)。这份文档把当前主线、旧流程、三维模型、论文图片、Git 大文件规则整理在一起，避免从零翻目录。
...
`docs/` 下的文档**文件名前缀就是阅读顺序**，直接 `ls docs/` 即可，不用记任何东西：

| 前缀 | 文档 | 什么时候读 |
|---|---|---|
| `10` | [项目说明书](docs/10-项目说明书.md) | 长期没碰项目，要恢复全局 |
| `20` `21` | [环境复现说明](docs/20-环境复现说明.md) · [运行入口说明](docs/21-运行入口说明.md) | 要跑代码 |
| `30` `31` | [参数定义规范](docs/30-参数定义规范.md) · [数据说明](docs/31-数据说明.md) | 写方法章节、核对符号 |
| `40` `41` | [论文证据链](docs/40-论文证据链.md) · [指标台账](docs/41-指标台账.md) | 写论文、填数字 |
| `50` | [决策记录](docs/50-决策记录.md) | 答辩问答、方法选型论证 |
| `90`+ | [AI协作上下文](docs/90-AI协作上下文.md) · [目录说明](docs/91-目录说明.md) · [目录当前审查](docs/92-目录当前审查.md) · [目录整理方案](docs/93-目录整理方案.md) · [目录迁移清单](docs/94-目录迁移清单.md) | 交给 AI 接手 / 整理目录 |

参数定义规范是论文符号表的唯一真源；决策记录是"为什么这么做"的唯一真源。旧六参数实验流程文档已归档到 `归档/旧论文材料/`，旧论文图件生成脚本已归档到 `归档/旧论文图件/`。
...
旧六参数模型结果、旧论文材料和历史答辩输出可以按 `docs/94-目录迁移清单.md` 逐项归档。
