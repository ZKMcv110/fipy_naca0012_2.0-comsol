...
"""
物理信息CNN 预测脚本
使用训练好的 PI-CNN-CBAM 模型对新的 CFD 算例进行 Nu/f 预测

用法：
  python 物理信息CNN_预测.py --case-id 42
  python 物理信息CNN_预测.py --all-test       # 预测全部测试集
"""
...
from 物理信息CNN_模型 import PICNNCBAM
...
def load_model(model_path, device, num_scalars=6):
    """加载训练好的模型权重"""
    model = PICNNCBAM(num_fields=3, num_scalars=num_scalars, output_size=2)
...
def prepare_input(case_id, data_dir, df, stats, img_size=224):
    ...
    for name in FIELD_PNGS:
        ...
        imgs.append(arr)
    images = torch.tensor(np.stack(imgs)).permute(0, 3, 1, 2).unsqueeze(0)
...
def predict_one(model, images, scalars, stats, device):
    ...
    with torch.no_grad():
        nu_f, _field = model(images.to(device), scalars.to(device), return_field=True)
        out = nu_f.cpu().numpy()[0]
...
def main():
    parser = argparse.ArgumentParser(description="使用物理信息CNN模型预测 Nu/f")
    parser.add_argument("--model-path", default=os.path.join("pi_cnn_nuf_results", "pi_cnn_nuf_best.pt"))
    parser.add_argument("--stats-path", default=os.path.join("pi_cnn_nuf_results", "pi_cnn_nuf_stats.json"))
