...
def plot_fig1_geometry_schematic():
    ...
    # 尺寸标注 (纯正 CAD 风格，添加延长边界线，使用细长箭头防重叠)
    # 纵向间距 Ts
    ax1.plot([0, 0], [-dy*1.1, -dy*1.2], 'k-', lw=1, alpha=0.5)
    ax1.plot([dx, dx], [-dy*1.1, -dy*1.2], 'k-', lw=1, alpha=0.5)
    ax1.annotate('', xy=(0, -dy*1.15), xytext=(dx, -dy*1.15), arrowprops=dict(arrowstyle='<->', color='black', lw=1.5, mutation_scale=12))
    ...
    ax1.annotate('', xy=(dx*2.8, 0), xytext=(dx*2.8, -dy), arrowprops=dict(arrowstyle='<->', color='black', lw=1.5, mutation_scale=12))
    ...
    ax1.annotate('', xy=(dx*2, dy*1.15), xytext=(dx*2 + Tad*chord, dy*1.15), arrowprops=dict(arrowstyle='<->', color='black', lw=1.5, mutation_scale=10))
    ...
    ax2.plot([0, chord_b], [0, 0], 'k-.', alpha=0.6, linewidth=1.2) # 弦线
    ax2.plot(x, yc, 'r--', alpha=0.8, linewidth=1.5, label='Camber Line') # 弯度线

    # 【完美CAD标注】1. 弦长 L
    ax2.plot([0, 0], [-0.05, -0.35], 'k-', alpha=0.5, lw=1)
    ax2.plot([chord_b, chord_b], [-0.05, -0.35], 'k-', alpha=0.5, lw=1)
    ax2.annotate('', xy=(0, -0.3), xytext=(chord_b, -0.3), arrowprops=dict(arrowstyle='<->', color='black', lw=1.5, mutation_scale=12))
    ...
    ax2.annotate('', xy=(chord_b*1.15, ymin_tb), xytext=(chord_b*1.15, ymax_tb), arrowprops=dict(arrowstyle='<->', color='black', lw=1.5, mutation_scale=12))
    ...
    ax2.annotate('', xy=(-0.1, 0), xytext=(-0.1, y_cmax), arrowprops=dict(arrowstyle='<->', color='red', lw=1.5, mutation_scale=12))
    ...
    ax2.annotate('', xy=(0, 0.3), xytext=(Twa_b*chord_b, 0.3), arrowprops=dict(arrowstyle='<->', color='black', lw=1.5, mutation_scale=12))
...
def plot_fig2_cnn_architecture_3d():
    ...
    ax.text(11, 8.5, "3D Architecture of the Multi-Modal Dual-Stream CNN with CBAM", 
    ...
    y1, z_base = 4.5, 20
    # 【重点修复：支路大标题上移】
    ax.text(0.5, y1 + 3.6, "Stream 1: CFD Physical Fields", color='#1d4ed8', fontweight='bold', fontsize=15)
    ...
    y2 = 1.0
    ax.text(0.5, y2 + 1.6, "Stream 2: Topology Scalars", color='#15803d', fontweight='bold', fontsize=15)
    ...
    y3 = 2.5
    ax.text(17.5, y3 + 2.8, "Fusion & Output", color='#6d28d9', fontweight='bold', fontsize=15)
    
    f1 = draw_cuboid(ax, x=17.5, y=y3-0.8, w=0.8, h=2.5, d=0.2, color='#a855f7', text_top='Concat', text_bottom='896', z=z_base-7)
    draw_3d_arrow(ax, p7, (17.5, y3+1.0)) 
    draw_3d_arrow(ax, s3, (17.5, y3-0.2)) 
    f2 = draw_cuboid(ax, x=19.3, y=y3-0.5, w=0.6, h=1.8, d=0.2, color='#9333ea', text_top='FC+Drop', text_bottom='512', z=z_base-8)
    draw_3d_arrow(ax, f1, (19.3, y3+0.4))
    f3 = draw_cuboid(ax, x=20.8, y=y3-0.1, w=0.2, h=0.8, d=0.2, color='#ef4444', text_top='Output', text_bottom='Nu, f (2)', z=z_base-9)
    draw_3d_arrow(ax, f2, (20.8, y3+0.3))

    plt.savefig('Fig2_CNN_Architecture_3D.png', dpi=600)
    plt.close()
    print("✅ 图 2 (文字已防重叠的 3D CNN 架构矢量图) 生成完毕！")
...
def plot_fig5_cnn_metrics():
    ...
    plt.close()
    print("✅ 图 5 (CNN 预测精度散点图与 Loss 曲线) 生成完毕！")

if __name__ == '__main__':
    print("🚀 开始批量生成全套高质量学术图表 (完美防遮挡 CAD 版)...")
    ...
    print("=========================================================")
    print("🎉 恭喜！所有图表均已重新生成完毕，已消除所有黑块与重叠！")